...
class GetFocusQualityUseCaseTest {
    ...
    @Test
    fun `all clean completed sessions produce Focused label with moderate volume`() = runTest {
    ...
    @Test
    fun `completed with interruption lowers score`() = runTest {
        val session = TestFixtures.testSession(
            id = "s1",
            startedAtEpochMillis = TestFixtures.jan15_2026_10am(),
            validFocusSeconds = 3600,
    ...
    @Test
    fun `label boundaries are correct`() = runTest {
    ...
    private fun assertLabel(score: Int, expected: FocusQualityLabel) {
