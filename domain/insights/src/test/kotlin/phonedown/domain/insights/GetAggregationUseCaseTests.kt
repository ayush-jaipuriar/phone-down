...
class GetBestHourUseCaseTest {
    ...
    @Test
    fun `no sessions returns null`() = runTest {
    ...
    @Test
    fun `single session returns its hour`() = runTest {
    ...
    @Test
    fun `abandoned sessions are excluded`() = runTest {
        val session = TestFixtures.testSession(
            id = "s1",
            startedAtEpochMillis = TestFixtures.jan15_2026_10am(),
...
class GetBestWeekdayUseCaseTest {
    ...
    @Test
    fun `single session returns its weekday`() = runTest {
        val session = TestFixtures.testSession(
            id = "s1",
            startedAtEpochMillis = TestFixtures.jan15_2026_10am(),
            validFocusSeconds = 1500,
...
class GetHistoryUseCaseTest {
    ...
    @Test
    fun `empty sessions returns empty list`() = runTest {
        val repo = FakeSessionRepository(emptyList())
...
class GetAdvancedInsightsUseCaseTest {
    ...
    @Test
    fun `no meaningful sessions returns null`() = runTest {
        val repo = FakeSessionRepository(emptyList())
        val clock = TestClock(TestFixtures.jan15_2026_10am())
    ...
    @Test
    fun `longest clean session is found correctly`() = runTest {
