...
class GetFocusQualityUseCase(
    ...
    suspend operator fun invoke(): FocusQualityResult? {
        ...
        val sessions =
            fetchSessions(sevenDaysAgo, now)
        ...
        val score =
            ((completionRate * 40f) + (cleanRatio * 25f) + (focusVolumeScore * 20f) + (interruptionScore * 15f))
                .toInt()
                .coerceIn(0, 100)
    ...
    companion object {
        fun activeSessionIds(sessions: List<FocusSession>): Set<String> = sessions.filter { it.validFocusSeconds > 0 }.map { it.id }.toSet()
        ...
        fun normalizeInterruptions(
            totalInterruptions: Float,
            sessionCount: Float,
        ): Float {
